...
FIELD_OFFSETS: Dict[str, Tuple[float, float]] = {
    ...
    "deposit_amount": (19, -6),
    "counter_offer_numbers": (1, -3),    # right 1 char (+6pt) from previous -5
    "buyer_counter_offer":   (-134, -3), # right 5 chars (+30pt) from previous -164
    "seller_agent": (48, -4),
    "escrow_company": (54, -4),
    "by_name": (20, -5),
    "subject_terms": (0, -2),
...
FIELD_FONT_SIZES: Dict[str, float] = {
    "seller_agent": 9.0,
    "counter_offer_numbers": 9.0,
    "buyer_counter_offer": 9.0,
}
...
def apply_template_alignments(coords: Dict[str, Tuple[float, float]]) -> Dict[str, Tuple[float, float]]:
    ...
    2) second_date: same vertical line (x) as acceptance_date (final rendered x);
       same horizontal line (y) as by_name / ESCROW_AGENT_NAME (final rendered y).
    """
    c = dict(coords)
