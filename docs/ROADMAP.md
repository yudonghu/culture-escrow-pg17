...
- [x] Web UI 错误提示按状态码分类显示 + 60s 超时保护（PR #36）
- [x] 修复 Caddyfile 缺少 `api.hydenluc.com` 配置（服务不可达）
- [x] 修复生产 CORS 白名单漏掉 `portal.cultureescrow.com`
...
#### 近期（改动小，优先做）
- [ ] Caddyfile 纳入版本控制（`deploy/` 目录，防止重建服务器时配置丢失）
