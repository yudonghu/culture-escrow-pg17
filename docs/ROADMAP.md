...
- [ ] 异步任务队列（触发时机：用户反馈提交后长时间无响应）
- [ ] Redis 幂等存储（触发时机：需要横向扩展到多台服务器）
- [ ] 多租户权限体系（触发时机：需要为不同 escrow 公司提供独立账号）
...
- [x] 留存与删除策略可执行
- [x] 幂等/重试机制已启用
- [x] staging 压测通过（100 req /health P95=241ms；25 req fill 限流第 21 次精准触发 429）
- [x] 回滚演练通过（staging 切 v1.0.0 服务正常启动，切回 main 恢复正常）
